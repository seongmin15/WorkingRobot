...
import com.wor.server.service.robotService;
...
public class socket_serverController {
    ...
    {
        int order_size = robotService.orderQueue.size();
//        log.info(ReservationService.orderQueue.toString());
//        log.info("size:" + order_size);

        for (Map<String, Object> order : robotService.orderQueue) {
    ...
    public void getFinishMessage(Map<String, Object> params) {
        ...
        robotService.processFinish(serial);
        if (params.get("type").toString().equals("elec")) {
            Map<String, Object> elec = new HashMap<>();
            List<Boolean> elecList = robotService.getElecList(serial);

            elec.put("robot_serial", serial);
            elec.put("type", "elec");
            elec.put("status", elecList);
            sendLocationMessage(elec);
        }
    }
}
